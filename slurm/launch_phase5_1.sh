#!/bin/bash
# Phase 5.1 — Extended HP combinations + batch size + change representation ablations.
#
# Three axes:
...
echo "Phase 5.1 — HP combinations + batch size + change representation ablations"
...
# bs=64: smaller batches → noisier gradients, may escape local minima; helps minority-class recall
submit_bs "hp_bs64"   "--batch_size 64"

# bs=256: larger batches → smoother loss landscape, lower wall time; may hurt minority recall
submit_bs "hp_bs256"  "--batch_size 256"

echo ""

# ── C) Change representation ablations ───────────────────────────────────────
echo "--- C) Change representation ablations ---"

# Remove fct_* one-hot on commit→fn edges (change type: MODIFY/ADD/DELETE/RENAME/REFACTOR)
...
# Tests: pure structural + embedding model — no explicit change encoding at all
...
# ── D) Code before / after / delta ablations ─────────────────────────────────
...
# Add code_before embedding (function.x: 776 → 1544 dims). Does BEFORE state help?
...
echo "  python scripts/aggregate_finals.py --prefix hp_c_  --output checkpoints/phase5_1_hp_summary.json"
echo "  python scripts/aggregate_finals.py --prefix abl_chg_ --output checkpoints/phase5_1_abl_summary.json"
echo "  python scripts/aggregate_finals.py --prefix abl_code_ --output checkpoints/phase5_1_code_abl_summary.json"
echo "  python scripts/aggregate_finals.py --prefix abl_      --output checkpoints/phase5_1_all_abl_summary.json"
